...
void OLED_ShowfloatNum(uint8_t x, uint8_t y, float Number, uint8_t intLength, uint8_t floatLength)
{
    ...
    {
        OLED_ShowChar(x + intLength - i, y, (int32_t)Number / OLED_Numexponent(10,i) % 10 + '0');
    }
    OLED_ShowChar(x + intLength + 1, y, '.');
    for (uint8_t i = 1; i < floatLength + 1; i++)
    {
        int32_t NumberFloat = Number * OLED_Numexponent(10,i);
        OLED_ShowChar(x + intLength + i + 1, y, NumberFloat % 10 + '0');
    }
}
