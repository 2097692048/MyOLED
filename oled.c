...
void OLED_Init(void)
{
    ...
    const uint8_t ssd1306_init_cmds[] = {
        ...
        0xD5,       // 设置显示时钟分频比/振荡器频率 (Set Display Clock Divide Ratio/Oscillator Frequency)
        0x80,       // 建议值：0x80 (recommended value)

        0xA8,       // 设置多路复用率 (Set Multiplex Ratio)
        0x3F,       // 对于128x64 OLED：64-1=63=0x3F (for 128x64 display)

        0xD3,       // 设置显示偏移 (Set Display Offset)
        0x00,       // 偏移值为0 (no offset)

        0x40,       // 设置显示起始行 (Set Display Start Line, 0x40-0x7F)

        0xA1,       // 设置段重映射 - 水平翻转 (Set Segment Re-map, horizontal flip)
        0xC8,       // 设置COM扫描方向 - 垂直翻转 (Set COM Output Scan Direction, vertical flip)

        0xDA,       // 设置COM硬件引脚配置 (Set COM Pins Hardware Configuration)
        0x12,       // 对于128x64屏幕：0x12，对于128x32屏幕：0x02 (for 128x64 display)

        0x81,       // 设置对比度控制 (Set Contrast Control)
        0xCF,       // 对比度值：0x00-0xFF，默认0x7F (contrast value: 0x00-0xFF, default 0x7F)

        0xD9,       // 设置预充电周期 (Set Pre-charge Period)
        0xF1,       // 建议值：0xF1 (recommended value)

        0xD8,       // 设置VCOMH取消选择电平 (Set VCOMH Deselect Level)
        0x30,       // 建议值：0x30 (recommended value)

        0xA4,       // 关闭整体显示 (Entire Display ON - resume)
        0xA6,       // 设置正常显示模式 (Set Normal Display, not inverse)

        0x8D,       // 设置电荷泵 (Charge Pump Setting)
        0x14,       // 0x14：使能电荷泵，0x10：禁用 (0x14: enable charge pump, 0x10: disable)

        0xAF,       // 开启显示 (Display ON)
    };
    HAL_I2C_Mem_Write(I2C_Dataout,OLED_I2C_ADDR,0x00,I2C_MEMADD_SIZE_8BIT,(uint8_t *) &ssd1306_init_cmds[0],30,OLED_TIMEOUT);
    HAL_Delay(100);
    HAL_Delay(100);
    OLED_Clear();
}
...
{
//    const uint8_t OLED_SetCursor[] = {
//        0x00 | (x & 0x0F),
//        0x10 | ((x & 0xF0) >> 4),
//        0xB0 | Page
//    };
//    OLED_WriteCmd(OLED_SetCursor, 3);
    HAL_I2C_Mem_Write(I2C_Dataout,OLED_I2C_ADDR,0x00,I2C_MEMADD_SIZE_8BIT, (uint8_t[]){0x00 | (x & 0x0F)}, 1, 100);
    HAL_I2C_Mem_Write(I2C_Dataout,OLED_I2C_ADDR,0x00,I2C_MEMADD_SIZE_8BIT, (uint8_t[]){0x10 | ((x & 0xF0) >> 4)}, 1, 100);
    HAL_I2C_Mem_Write(I2C_Dataout,OLED_I2C_ADDR,0x00,I2C_MEMADD_SIZE_8BIT, (uint8_t[]){0x10 | 0xB0 | Page}, 1, 100);
}
/*
 *  函数名：OLED_Clear
 *  功能描述：OLED清屏
